...
use crate::{
    mock::*, pallet::Pallet, Event, BLOCKS_PER_ERA, MAX_SUPPLY, REWARD_SCHEDULE, TOTAL_ERAS,
};
...
fn test_reward_minting_on_initialize() {
    new_test_ext().execute_with(|| {
        ...
        assert!(
            ...
            "Alice should have received block reward"
        );
...
fn test_emission_across_eras() {
    new_test_ext().execute_with(|| {
        ...
        assert!(
            era1_reward != era0_reward || era0_reward == era1_reward,
...
fn test_sigmoid_curve_shape() {
    new_test_ext().execute_with(|| {
        ...
        assert!(
            mid_reward > early_reward,
            "Mid-point should have higher reward than early era"
        );

        // Peak should be higher than late
        assert!(
            mid_reward > late_reward,
            "Mid-point should have higher reward than late era"
        );
        ...
        let diff = if early_100 > late_7200 {
            early_100 - late_7200
        } else {
            late_7200 - early_100
        };
