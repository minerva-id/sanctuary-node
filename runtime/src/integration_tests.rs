...
fn integration_emission_constants_match_runtime() {
    ...
    assert_eq!(pallet_emission::MAX_SUPPLY, tesserax_constants::MAX_SUPPLY);
}
...
fn integration_dev_endowment_distribution() {
    ...
    assert!(
        diff < TSRX * 10,
        "Dev endowments × 4 should ≈ genesis supply"
    );
}
...
fn integration_emission_sigmoid_properties() {
    ...
    assert!(
        era_mid > era_100,
        "Mid-point should have higher reward than era 100"
    );
    assert!(
        era_mid > era_late,
...
fn integration_version_info() {
    ...
    assert_eq!(
        VERSION.spec_name,
        alloc::borrow::Cow::Borrowed("tesserax-runtime")
    );
    assert_eq!(
        VERSION.impl_name,
        alloc::borrow::Cow::Borrowed("tesserax-runtime")
